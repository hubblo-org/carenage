...
#[test]
fn it_fails_when_giving_an_invalid_time_step_with_start_command() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn it_prints_error_message_when_it_fails_to_request_boagent() -> Result<(), Box<dyn std::error::Error>> {
    let mut cmd = Command::cargo_bin("carenage")?;

    cmd.arg("start");
    cmd.assert().success().stdout(contains("Failed to query Boagent"));
